...
.SYNOPSIS
  ...
  powershell -NoProfile -ExecutionPolicy Bypass -File "C:\000-Essential-tasks\zzz-backups_02.ps1" -Target_Drives_List "D: E: F: G: T: I: Y:" -Headroom_PCT 35 -Debug True
...
# Make debug flag available to functions
$script:Debug = $Debug
...
function Trace([string]$Message) {
    if ($script:Debug) {
        Write-Host ("[DEBUG] {0}" -f $Message) -ForegroundColor DarkGray
    }
}
...
function Dump-Object($Object, [string]$Label = "") {
    ...
    if ($Label) {
        Write-Host ("[DEBUG] {0} =" -f $Label) -ForegroundColor DarkGray
    }
    try {
        $Object | Format-List * | Out-String -Width 500 |
            ForEach-Object { Write-Host $_ -ForegroundColor DarkGray }
    } catch {
        Write-Host "[DEBUG] <unprintable object>" -ForegroundColor DarkGray
    }
}
